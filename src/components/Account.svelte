...
let answer: { target: string; data: string } | undefined = $state(undefined);
...
async function refreshOffers() {
	try {
		const response = await fetch("http://localhost:4200/webrtc/offers", {
			method: "GET",
			mode: "cors",
			headers: {
...
async function refreshAnswers() {
	...
	try {
		const response = await fetch(
			"http://localhost:4200/webrtc/answer/" + surname,
			{
				method: "GET",
				mode: "cors",
				headers: {
		...
		const data = await response.json();
		if (answer && answer.target === data.target && answer.data === data.data) {
			return;
		}
		if (data.username === surname) {
			...
			const accept = confirm(`Accepter de jouer avec ${data.target} ?`);
			if (!accept) {
				return;
			}
...
function join(other: { username: string; offer: string }) {
	...
	localConnection
		...
		.then(() => {
			...
			fetch("http://localhost:4200/webrtc/answer", {
				method: "POST",
				mode: "cors",
				headers: {
					"Content-Type": "application/json",
